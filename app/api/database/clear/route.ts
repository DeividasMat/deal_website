...
async function runDuplicateCleanup(dryRun: boolean = true) {
  ...
  for (let i = 0; i < allArticles.length; i++) {
    ...
    if (similarArticles.length > 1) {
      ...
      duplicateGroups.push({
        ...
        reason: `Similar titles detected (high similarity)`
      });
    }
    ...
    return NextResponse.json({
      ...
      results: {
        ...
        details: duplicateGroups.map(group => ({
          keepTitle: group.keep.title.substring(0, 60) + '...',
          keepId: group.keep.id,
          deleteCount: group.delete.length,
          deleteIds: group.delete.map((a: any) => a.id),
          reason: group.reason
        }))
      }
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'cleanup-duplicates') {
      return await runDuplicateCleanup(dryRun);
    }
    
    // Original clear database logic
    const { confirm } = body;
    ...
    if (confirm !== 'DELETE_ALL') {
      return NextResponse.json({
        ...
        message: 'Send { "confirm": "DELETE_ALL" } to confirm database clearing OR { "action": "cleanup-duplicates", "dryRun": true/false }'
      }, { status: 400 });
    }
